...
export function Modal({title,description,close,children,wide=false,compact=false}:{title:string;description?:string;close:()=>void;children:React.ReactNode;wide?:boolean;compact?:boolean}){const compactTask=compact||title==='Новая задача';const compactResource=title==='Сохранить материал';return <Dialog open onOpenChange={v=>{if(!v)close()}}><DialogContent className={'app-dialog '+(wide?'wide ':'')+(compactTask?'compact-task ':'')+(compactResource?'compact-resource':'')}><DialogTitle>{title}</DialogTitle><DialogDescription>{description||'Изменения сохраняются в вашем личном пространстве.'}</DialogDescription>{children}</DialogContent></Dialog>}
...
export function SafeLink({url,children}:{url:string;children:React.ReactNode}){let valid=false;try{valid=['http:','https:'].includes(new URL(url).protocol)}catch{}return valid?<a className="resource-link" href={url} target="_blank" rel="noopener noreferrer"><span data-no-translate>{children}</span><ExternalLink size={14}/></a>:<span>{children}</span>}
export function ResourceLink({r}:{r:Resource}){const {language}=useApp();return <div className="resource-line"><ResourceIcon type={r.type} small/><div><SafeLink url={r.url}>{r.title}</SafeLink><small data-no-translate>{translateText(r.type,language)}{r.why?' · '+r.why:''}</small></div></div>}

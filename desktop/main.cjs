...
function createCompanion(){
  ...
  companion.on('move',()=>{clearTimeout(moveTimer);moveTimer=setTimeout(rememberPosition,200)});
...
app.whenReady().then(async()=>{
  ...
  const icon=nativeImage.createFromPath(path.join(__dirname,'icon.png')).resize({width:16,height:16});tray=new Tray(icon);tray.setToolTip('FocusBase');tray.on('double-click',()=>showMain());updateMenu();
